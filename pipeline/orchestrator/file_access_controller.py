...
class FileAccessController:
    ...
    ACCESS_RULES: Dict[AgentType, Dict[str, List[str]]] = {
        ...
        AgentType.SCENARIO_BUILDER: {
            # Blind building: cannot see tests/ or the oracle files (known_attacks.yaml,
            # for_attack_verifier.md). Builder must construct the stack from public
            # spec + builder hints only.
            'read_allowed': ['*'],
            'write_allowed': ['*'],
            'denied': [
                '.logs/*',
        ...
        # === Runtime evaluation ===
        # FUZZER is the agent under evaluation. It must NOT see the oracle.
        AgentType.FUZZER: {
            'read_allowed': ['*'],
            'write_allowed': [
                'pocs/*',
            ...
            'denied': [
                '.logs/*',
                # Oracle / hint files hidden from fuzzer (toggle for_fuzzer.md via config)
                '.agent_state/analyzer_output/known_attacks.yaml',
                '.agent_state/analyzer_output/for_attack_verifier.md',
                '.agent_state/analyzer_output/for_fuzzer.md',
                '.agent_state/verifier_output/*',
                'tests/test_known_attacks.py',
            ],
        },

        AgentType.KNOWN_ATTACKER: {
            'read_allowed': [
                '*',
            ],
            'write_allowed': [
        ...
        AgentType.POC_JUDGER: {
            'read_allowed': ['*'],
            'write_allowed': [
                '.agent_state/poc_judger_output/*',
                '.agent_state/poc_judger-res.xml',
            ],
            'denied': [],
        },

        # === Retained extension agents ===
        AgentType.CHANGER: {
            'read_allowed': ['*'],
            'write_allowed': ['*'],
            'denied': ['.logs/*'],
    ...
    def can_read_file(cls, agent_type: AgentType, file_path: str) -> bool:
        ...
        # Use case-insensitive matching for denied patterns (e.g., Fix, PATCH)
        norm_path_lower = norm_path.lower()
        for pattern in denied_patterns:
            if fnmatch.fnmatch(norm_path_lower, pattern.lower()):
                return False
...
def check_builder_blind_testing(working_dir: Path) -> Dict[str, bool]:
    ...
    results = {
        ...
        'builder_can_read_known_attacks': FileAccessController.can_read_file(
            AgentType.SCENARIO_BUILDER,
            '.agent_state/analyzer_output/known_attacks.yaml'
        ),
        'fuzzer_can_read_known_attacks': FileAccessController.can_read_file(
            AgentType.FUZZER,
            '.agent_state/analyzer_output/known_attacks.yaml'
        ),
        'fuzzer_can_read_test_known_attacks': FileAccessController.can_read_file(
            AgentType.FUZZER,
            'tests/test_known_attacks.py'
        ),
        'verifier_can_read_tests': FileAccessController.can_read_file(
            AgentType.ATTACK_VERIFIER,
            'tests/test_known_attacks.py'
        ),
        'verifier_can_write_tests': FileAccessController.can_write_file(
            AgentType.ATTACK_VERIFIER,
            'tests/test_known_attacks.py'
        ),
    }
...
def _get_denial_reason(agent_type: AgentType, file_path: str, operation: str = "access") -> str:
    ...
    if agent_type == AgentType.SCENARIO_BUILDER:
        if file_path.startswith('tests') or 'known_attacks' in file_path or 'for_attack_verifier' in file_path:
            return f"To prevent oracle leakage, you cannot view '{file_path}'. Build the stack from public.md and for_scenario_builder.md only."
        if file_path.startswith('task-deps/') and ('fix' in file_path.lower() or file_path.endswith('.patch')):
            return f"To prevent data leakage, you cannot view '{file_path}'. This file contains fix/patch info that must not influence the env."

    if agent_type == AgentType.FUZZER:
        if 'known_attacks' in file_path or 'for_attack_verifier' in file_path or \
           'for_fuzzer' in file_path or file_path.endswith('test_known_attacks.py') or \
           file_path.startswith('.agent_state/verifier_output'):
            return f"Anti-cheat: '{file_path}' is the evaluation oracle and is hidden from the fuzzer agent under test."
...
def create_file_access_hooks(
    ...
    def _allow_response(hook_event_name: str) -> Dict[str, Any]:
        """Create a properly formatted allow response for PreToolUse hooks.

        This explicitly grants permission so tools like WebSearch/WebFetch
        don't require interactive approval. Essential for running as root
        where bypassPermissions mode is blocked.
        """
        return {
            'hookSpecificOutput': {
                'hookEventName': hook_event_name,
    ...
    async def pre_tool_use_hook(
        ...
        if tool_name == 'Bash':
            ...
            # Allow Bash command
            return _allow_response(hook_event_name)

        # Determine if this is a read or write operation
        is_read = tool_name in READ_TOOLS
        is_write = tool_name in WRITE_TOOLS

        # Non-file tools are allowed (WebSearch, WebFetch, Task, etc.)
