...
When running docker commands directly, set proxy first.
...
For network operations inside the container, pass proxy as environment variables:

```bash
docker exec <container_id> bash -c "export http_proxy=http://<YOUR_PROXY_HOST>:<YOUR_PROXY_PORT> && \
                                    export https_proxy=http://<YOUR_PROXY_HOST>:<YOUR_PROXY_PORT> && \
                                    pip install pytest"
```
